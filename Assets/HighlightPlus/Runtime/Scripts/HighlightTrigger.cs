...
namespace HighlightPlus {
    ...
    public class HighlightTrigger : MonoBehaviour {
        ...
        // Bỏ qua vài frame sau khi Game view đổi kích thước (Device Simulator)
        // để tránh pixelWidth/pixelHeight = 0 và input trả về NaN.
        const int SKIP_FRAMES_AFTER_RESIZE = 2;
        int _skipFramesLeft = 0;
        int _lastPW = -1, _lastPH = -1;

        bool IsCamReady(Camera cam)
        {
            if (cam == null) return false;
            int pw = cam.pixelWidth;
            int ph = cam.pixelHeight;

            // Phát hiện thay đổi kích thước viewport (Simulator đổi máy/đổi hướng)
            if (pw != _lastPW || ph != _lastPH)
            {
                _lastPW = pw; _lastPH = ph;
                _skipFramesLeft = SKIP_FRAMES_AFTER_RESIZE;
            }

            if (_skipFramesLeft > 0)
            {
                _skipFramesLeft--;
                return false; // bỏ qua frame này
            }

            // Không raycast khi kích thước chưa hợp lệ
            if (pw <= 0 || ph <= 0) return false;
            return true;
        }

        // Lấy pointer (chuột/ngón tay/center of screen) và lọc NaN/Infinity + clamp vào viewport.
        // Trả về false nếu không dùng được.
        bool TryGetSafePointer(Camera cam, RayCastSource src, out Vector3 sp)
        {
            sp = default;
            if (cam == null) return false;

            if (src == RayCastSource.MousePosition)
            {
                sp = InputProxy.mousePosition; // plugin đã wrap input cho bạn
            }
            else
            {
                sp = new Vector3(cam.pixelWidth * 0.5f, cam.pixelHeight * 0.5f, 0f);
            }
            ...
            // Clamp vào viewport hợp lệ để tránh out-of-frustum khi resize
            sp.x = Mathf.Clamp(sp.x, 0, cam.pixelWidth);
            sp.y = Mathf.Clamp(sp.y, 0, cam.pixelHeight);
        ...
        IEnumerator DoRayCast() {
            ...
            while (triggerMode == TriggerMode.RaycastOnThisObjectAndChildren)
            {
                ...
                // ✅ NEW: bỏ qua frame khi camera/viewport chưa sẵn sàng (Simulator đang đổi kích thước)
                if (!IsCamReady(raycastCamera))
                {
                    yield return null;
                    continue;
                }
...
#endif
                    // ✅ NEW: lấy screenPos đã được lọc
                    Vector3 sp;
                    if (!TryGetSafePointer(raycastCamera, raycastSource, out sp))
                    {
                        yield return null;
                        continue; // bỏ qua frame có input lỗi/NaN
                    }
                    ray = raycastCamera.ScreenPointToRay(sp);
                }
                else
                {
                    // CameraDirection
